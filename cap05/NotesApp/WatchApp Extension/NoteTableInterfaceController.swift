...
class NoteTableInterfaceController: WKInterfaceController {
    ...
    override func handleUserActivity(userInfo: [NSObject : AnyObject]?) {
        if let noteID = userInfo?[kHandoffNoteID] as? String {
            if let noteIndex = NotesStorage.sharedInstance.noteItemIndexWithUUID(NSUUID(UUIDString: noteID)!) {
                self.pushControllerWithName("NoteDetail", context: NotesStorage.sharedInstance.notes[noteIndex])
            }
        }
    }
