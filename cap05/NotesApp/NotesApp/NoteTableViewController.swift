...
class NoteTableViewController: UITableViewController {
    ...
    override func restoreUserActivityState(activity: NSUserActivity) {
        print("restoreUserActivityState \(activity.userInfo?[kHandoffNoteID])")
        
        if let noteID = NSUUID(UUIDString: activity.userInfo?[kHandoffNoteID] as! String) {
            if let noteIndex = NotesStorage.sharedInstance.noteItemIndexWithUUID(noteID) {
                self.performSegueWithIdentifier("edit", sender: NotesStorage.sharedInstance.notes[noteIndex])
            }
        }
    }
